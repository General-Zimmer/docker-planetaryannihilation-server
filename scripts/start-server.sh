...
if [ "${GAME_STREAM}" == "stable" ]; then
	...
	${SERVER_DIR}/server --game-mode ${GAME_MODE} --server-name "${SERVER_NAME}" --mt-enabled --max-players ${MAX_PLAYERS} --headless ${GAME_PARAMS} --allow-lan --port ${GAME_PORT} --password ${SERVER_PASSWORD} ${GAME_PARAMS}
fi
